    ...
    public class MembershipService : IMembershipService
    {
        ...
        public ValidUserContext ValidateUser(string username, string password)
        {
            ...
            if (user != null && IsUserValid(user, password))
            {
                ...
                var roles = userRoles as Role[] ?? userRoles.ToArray();
                ...
                userCtx.Principal = new GenericPrincipal(identity, roles.Select(x => x.Name).ToArray());
            }
        ...
        public bool RemoveFromRole(string username, string role)
        {
            ...
            if (user != null && roleEntity != null)
            {
                var userInRole = _userInRoleRepository.GetAll()
                    .FirstOrDefault(x => x.RoleKey == roleEntity.Key && x.UserKey == user.Key);
        ...
        {
            var users = _userRepository.Paginate(pageIndex, pageSize, x => x.Key);
        ...
        private IEnumerable<Role> GetUserRoles(Guid userKey)
        {
            ...
            if (userInRoles != null && userInRoles.Any())
            {
